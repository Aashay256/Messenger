var chat_data = {}, user_uuid, chatHTML = '', chat_uuid = "", userList = [];
						...
						$.each(users, function(index, value){
							...
							if (user_uuid != value.uuid) {
								...
								usersHTML += '<div class="user" uuid="'+value.uuid+'">'+
											'<div class="user-image"><img src="userimages/'+value.impath+'" height="100%" width="100%"/></div>'+
											'<div class="user-details">'+
												'<span><strong>'+ value.fullname+'<span class="count"></span></strong></span>'+
												'<span>Last Login '+value.lastlogmessage+'</span>'+
											'</div>'+
										'</div>';
			...
			$.ajax({
				...
				success : function(resposne){
					...
					chat_data = {
						...
						user_1_image: resp.message.user_1_image,
						user_2_image: resp.message.user_2_image,
						user_1_name : '',
						user_2_name : name
					};
						...
						querySnapshot.forEach(function(doc){
							...
							if (doc.data().user_1_uuid == user_uuid) {
								...
								chatHTML += '<div class="message-block">'+
												'<div class="user-icon"><img src="userimages/'+chat_data.user_1_image+'" height="100%" width="100%"/></div>'+
												'<div class="message">'+ doc.data().message +'</div>'+
											'</div>';

							}else{
								chatHTML += '<div class="message-block received-message">'+
												'<div class="user-icon"><img src="userimages/'+chat_data.user_2_image+'" height="100%" width="100%"/></div>'+
												'<div class="message">'+ doc.data().message +'</div>'+
											'</div>';
							}
		...
		$(".send-btn").on('click', function(){
			...
			if(message != ""){
				...
				.catch(function(error) {
          		...
          		$.ajax({
            		url:"./chatlogs.php", //the page containing php script
            		type: "POST", //request type,
           			data: {'user_1':user_uuid, 'user_2': chat_data.user_2_uuid, 'message': message, 'time': timed},
		...
		/*function create (user_1_uuid,user_2_uuid,message,time) {
          $.ajax({
            url:"./chatlogs.php", //the page containing php script
            type: "post", //request type,
           data: {user_1_uuid , user_2_uuid , message , time}
